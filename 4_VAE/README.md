...
**Графики:**
- `01_class_distribution.png` — распределение классов по обучающей и тестовой выборкам.
- `01_data_preview.png` — примеры исходных изображений и их аугментированных версий.

![Распределение классов](output/figures/01_class_distribution.png)
![Примеры данных и аугментация](output/figures/01_data_preview.png)
...
**Графики:**
- `02_ae_loss_curve.png` — кривая обучения (BCE).
- `02_ae_reconstruction.png` — оригинал и реконструкция AE для 10 примеров.

![Кривая обучения AE](output/figures/02_ae_loss_curve.png)
...
**Графики:**
- `03_vae_loss_curve_dim2.png`, `03_vae_loss_curve_dim8.png`, `03_vae_loss_curve_dim16.png` — кривые BCE и KL для каждой размерности.
- `03_vae_reconstruction_dim2.png`, `03_vae_reconstruction_dim8.png`, `03_vae_reconstruction_dim16.png` — реконструкции VAE при разных размерностях.

![Кривые обучения VAE dim=2](output/figures/03_vae_loss_curve_dim2.png)
![Кривые обучения VAE dim=8](output/figures/03_vae_loss_curve_dim8.png)
![Кривые обучения VAE dim=16](output/figures/03_vae_loss_curve_dim16.png)

![Реконструкция VAE dim=2](output/figures/03_vae_reconstruction_dim2.png)
![Реконструкция VAE dim=8](output/figures/03_vae_reconstruction_dim8.png)
![Реконструкция VAE dim=16](output/figures/03_vae_reconstruction_dim16.png)
...
**Графики:**
- `04_random_generation.png` — сгенерированные цифры и оригиналы.
- `04_interpolation.png` — интерполяция между двумя точками.
- `04_latent_grid.png` — сетка латентного пространства.
- `04_noise_experiment.png` — влияние шума.

![Случайная генерация VAE](output/figures/04_random_generation.png)
![Интерполяция VAE](output/figures/04_interpolation.png)
![Сетка латентного пространства](output/figures/04_latent_grid.png)
![Влияние шума](output/figures/04_noise_experiment.png)
...
- `05_reconstruction_compare.png` — визуальное сравнение реконструкций всех методов (подписи содержат MSE).
- `05_latent_space_compare.png` — сравнение 2D латентных пространств PCA и VAE.

![Объяснённая дисперсия PCA](output/figures/05_pca_explained_variance.png)
![Сравнение MSE](output/figures/05_compare_mse.png)
![Сравнение реконструкций](output/figures/05_reconstruction_compare.png)
![Латентные пространства PCA vs VAE](output/figures/05_latent_space_compare.png)
...
- По сравнению с обычным VAE (8d, MSE=0.018880) β‑VAE показал немного лучшую реконструкцию, но основное преимущество лежит в более развязанном (disentangled) латентном пространстве.
...
![Генерация β‑VAE](output/figures/06_generation.png)
![Латентное пространство β‑VAE](output/figures/06_latent_space.png)
![Общее сравнение MSE с β‑VAE](output/figures/06_compare_all.png)
...
- Интерполяция в латентном пространстве GAN также демонстрирует плавные переходы между разными цифрами.
...
![Интерполяция GAN](output/figures/07_gan_interpolation.png)
...
**Финальные графики:**
- `08_final_mse_comparison.png` — сводная столбчатая диаграмма MSE всех методов.
- `08_vae_vs_gan.png` — прямое сравнение образцов, сгенерированных VAE (2d) и GAN.

![Финальное сравнение MSE](output/figures/08_final_mse_comparison.png)
![VAE vs GAN генерация](output/figures/08_vae_vs_gan.png)
